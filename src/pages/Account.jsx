...
function QuestionRow({ q }) {
  return (
    ...
    >
      <div className="shrink-0 flex flex-col items-center gap-1 pt-0.5 min-w-[48px]">
  ...
  return (
    <div className="flex min-h-screen bg-white dark:bg-gray-950">
      ...
      <div className="flex-1 animate-pulse min-w-0">
        <div className="h-52 bg-linear-to-br from-blue-500 to-indigo-600" />
        <div className="px-4 sm:px-8 -mt-12 space-y-6">
          <div className="flex items-end gap-4">
            <div className="w-24 h-24 rounded-full bg-gray-200 dark:bg-gray-700 ring-4 ring-white dark:ring-gray-950" />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-0">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-20 bg-gray-50 dark:bg-gray-900" />
            ))}
          </div>
        </div>
      </div>
...
export default function Account() {
  ...
  const activities = [
    ...questions.map((q) => ({
      ...
      link: `/question/${q.id}`, icon: FiHelpCircle, color: "text-emerald-500 bg-emerald-50 dark:bg-emerald-500/20",
    })),
    ...comments.map((c) => ({
      ...
      link: `/question/${c.postId || ""}`, icon: FiMessageSquare, color: "text-purple-500 bg-purple-50 dark:bg-purple-500/20",
    })),
  ...
  return (
    <div className="flex min-h-screen bg-white dark:bg-gray-950 transition-colors duration-300">
      ...
      <div className="flex-1 overflow-y-auto min-w-0">
        ...
        {/* ── HERO BANNER ─────────────────────────────────────────────── */}
        <div className="relative h-48 sm:h-56 bg-linear-to-br from-blue-600 via-indigo-600 to-violet-700 overflow-hidden">
          <div className="absolute -top-12 -right-12 w-64 h-64 bg-white/5 rounded-full" />
          <div className="absolute -bottom-20 -left-12 w-80 h-80 bg-white/5 rounded-full" />
          <div className="absolute top-4 right-4 flex items-center gap-2 bg-white/15 backdrop-blur-sm px-3 py-1.5 rounded-full">
            <FiCalendar className="w-3.5 h-3.5 text-white/80" />
            <span className="text-white/80 text-xs font-semibold">Member since {memberSince}</span>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* ── PROFILE INFO ROW ───────────────────────────────────────── */}
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 -mt-12 pb-6">
            <div className="flex items-end gap-4">
              {/* Avatar */}
              <div className="relative group shrink-0">
                <div className="w-24 h-24 rounded-full overflow-hidden ring-4 ring-white dark:ring-gray-950">
                  {avatarSrc ? (
                    <img src={avatarSrc} alt="Avatar" className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center bg-linear-to-br from-blue-500 to-indigo-600 text-white text-3xl font-black">
                      {initials}
                    </div>
                  )}
                </div>
              ...
              {/* Name & email */}
              <div className="pb-1">
                <h1 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white">{profile.displayName}</h1>
          ...
          {/* Bio */}
          {profile.bio ? (
            <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed mb-6 max-w-2xl">{profile.bio}</p>
          ) : (
            <button
              onClick={() => setIsEditing(true)}
              className="mb-6 text-sm text-blue-500 hover:text-blue-600 flex items-center gap-1.5 transition-colors"
            >
          ...
          {/* ── STATS ROW ─────────────────────────────────────────────── */}
          <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-gray-100 dark:divide-gray-800 bg-gray-50 dark:bg-gray-900 rounded-2xl mb-6 overflow-hidden">
            {[
              { icon: FiHelpCircle,   label: "Questions",     value: questions.length,        color: "text-blue-500" },
              { icon: FiMessageSquare,label: "Answers",        value: comments.length,         color: "text-purple-500" },
              { icon: FiStar,         label: "Reputation",     value: profile?.reputation ?? 0, color: "text-amber-500" },
              { icon: FiEye,          label: "Profile Views",  value: profile?.views ?? 0,      color: "text-emerald-500" },
            ].map((s) => (
          ...
          {/* ── TABS SECTION ────────────────────────────────────────────── */}
          <div className="bg-white dark:bg-gray-900 rounded-none sm:rounded-2xl overflow-hidden mb-8">
            {/* Tab bar */}
            <div className="flex border-b border-gray-100 dark:border-gray-800 overflow-x-auto scrollbar-hide">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`flex-1 min-w-[80px] sm:min-w-[110px] flex items-center justify-center gap-1.5 px-3 sm:px-4 py-3 sm:py-4 text-xs sm:text-sm font-semibold transition-all border-b-2 -mb-px whitespace-nowrap ${
                    activeTab === tab.id
                ...
                >
                  ...
                  <span className={`text-[10px] sm:text-[11px] px-1.5 py-0.5 rounded-full font-bold ${
                    activeTab === tab.id ? "bg-blue-500 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500"
                  }`}>
                    {tab.count}
                  </span>
                </button>
                ...
                <div>
                  {questions.length === 0 ? (
                    <div className="py-16 text-center">
                      ...
                      <Link to="/ask" className="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/20">
                        Ask a Question
                      </Link>
                    </div>
                  ) : (
                    <div className="divide-y divide-gray-100 dark:divide-gray-800 -mx-4 sm:-mx-6">
                    ...
                    <div className="space-y-2">
                      {comments.map((c) => (
                        <Link key={c.id} to={`/question/${c.postId}`}
                          className="group flex items-start gap-3 p-4 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-all"
                        >
                          <div className="w-8 h-8 rounded-lg bg-purple-100 dark:bg-purple-900/20 flex items-center justify-center shrink-0 mt-0.5">
                          ...
                          <FiExternalLink className="w-4 h-4 text-gray-300 group-hover:text-blue-400 shrink-0 transition-colors" />
                        </Link>
                      ))}
                    </div>
                  )}
              ...
              {activeTab === "activity" && (
                <div>
                  {activities.length === 0 ? (
                    <div className="py-16 text-center">
                      <div className="w-14 h-14 rounded-full bg-gray-50 dark:bg-gray-800 flex items-center justify-center mx-auto mb-4">
                        <FiActivity className="w-7 h-7 text-gray-300 dark:text-gray-600" />
                      </div>
                      <p className="text-gray-400 dark:text-gray-500 font-medium">No activity yet</p>
                    </div>
                  ) : (
                    <div className="relative border-l-2 border-gray-100 dark:border-gray-800 ml-4 space-y-5">
                      {activities.map((act) => (
                        <div key={act.id} className="relative pl-8 group">
                          <div className={`absolute -left-[15px] top-2 w-7 h-7 rounded-full flex items-center justify-center ${act.color}`}>
                            <act.icon className="w-3 h-3" />
                          </div>
                          <div className="rounded-xl p-4 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700/60 transition-colors">
                            ...
                            <Link to={act.link} className="block rounded-lg p-3 hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-colors">
                              <p className="text-gray-500 dark:text-gray-400 text-sm line-clamp-2 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 16 }}
              ...
              <div className="flex items-center justify-between px-8 py-5 border-b border-gray-100 dark:border-gray-800">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5 uppercase tracking-widest">Identity & Bio</p>
                </div>
              ...
              <form onSubmit={handleSave} className="px-8 py-6 space-y-5">
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" required value={formData.displayName}
                      ...
                      className="w-full pl-11 pr-4 py-3.5 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-2xl text-gray-900 dark:text-white font-semibold placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all"
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea rows={4} value={formData.bio}
                    ...
                    className="w-full px-4 py-3.5 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-2xl text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all resize-none"
                ...
                <div className="flex items-center gap-3 pt-2">
                  ...
                  <button type="submit" disabled={isUpdating} className="flex-[1.5] flex items-center justify-center gap-2 py-3 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-2xl transition-all shadow-lg shadow-blue-500/25 disabled:opacity-60">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-950/50 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="p-6 flex flex-col items-center gap-5">
                <div className="relative">
                  <div className="w-28 h-28 rounded-2xl overflow-hidden border-4 border-white dark:border-gray-800 shadow-xl">
                    {previewImage ? <img src={previewImage} alt="Preview" className="w-full h-full object-cover" />
                      ...
                      : <div className="w-full h-full bg-linear-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-3xl font-black">{initials}</div>}
                  </div>
                  {isUploading && <div className="absolute inset-0 bg-black/40 rounded-2xl flex items-center justify-center"><div className="w-8 h-8 border-4 border-white/30 border-t-white rounded-full animate-spin" /></div>}
                </div>
                <label
                  onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }}
                  onDragLeave={(e) => { e.preventDefault(); setIsDragging(false); }}
                  onDrop={(e) => { e.preventDefault(); setIsDragging(false); if (e.dataTransfer.files[0]) processFile(e.dataTransfer.files[0]); }}
                  className={`w-full py-7 flex flex-col items-center justify-center border-2 border-dashed rounded-2xl cursor-pointer transition-all gap-2 ${isDragging ? "border-blue-500 bg-blue-50 dark:bg-blue-900/20" : "border-gray-200 dark:border-gray-700 hover:border-blue-400 hover:bg-gray-50 dark:hover:bg-gray-800/50"}`}>
                  <div className="p-2.5 bg-blue-50 dark:bg-blue-900/30 text-blue-500 rounded-full"><FiUploadCloud className="w-5 h-5" /></div>
                  <p className="text-sm font-bold text-gray-700 dark:text-gray-200">{selectedFile ? selectedFile.name : "Choose a new photo"}</p>
                  <p className="text-[11px] text-gray-400 uppercase tracking-wider">Or drag and drop</p>
                  <input type="file" className="hidden" accept="image/*" onChange={(e) => processFile(e.target.files[0])} disabled={isUploading} />
                </label>
              </div>
              <div className="px-6 pb-6 flex gap-3">
                <button onClick={closeAvatarModal} disabled={isUploading} className="flex-1 py-3 text-sm font-bold text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-2xl transition-all">Cancel</button>
                <button onClick={handleUpload} disabled={!selectedFile || isUploading} className="flex-2 flex items-center justify-center gap-2 py-3 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-sm font-bold rounded-2xl transition-all shadow-lg disabled:opacity-50">Update Photo</button>
              </div>
            </motion.div>
          </div>
        )}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 16 }}
              ...
              <div className="flex items-center justify-between px-8 py-5 border-b border-gray-100 dark:border-gray-800">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5 uppercase tracking-widest">Protect Your Account</p>
                </div>
              ...
              <form onSubmit={handleChangePassword} className="px-8 py-6 space-y-5">
                <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/50 rounded-2xl flex items-start gap-3">
                  ...
                  <p className="text-xs text-amber-900/70 dark:text-amber-200/70 leading-relaxed">New password must be at least <strong className="text-amber-600 dark:text-amber-400">8 characters</strong> including letters and numbers.</p>
                </div>
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input type={showPw.old ? "text" : "password"} required value={pwForm.oldPassword}
                      ...
                      className="w-full pl-11 pr-12 py-3.5 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-2xl text-gray-900 dark:text-white font-semibold placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input type={showPw.new ? "text" : "password"} required value={pwForm.newPassword}
                        ...
                        className="w-full px-4 py-3.5 pr-11 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-2xl text-gray-900 dark:text-white font-semibold placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input type={showPw.confirm ? "text" : "password"} required value={pwForm.confirmedNewPassword}
                        ...
                        className={`w-full px-4 py-3.5 pr-11 bg-gray-50 dark:bg-gray-800/50 border rounded-2xl text-gray-900 dark:text-white font-semibold placeholder-gray-400 focus:outline-none focus:ring-4 transition-all ${
                          pwForm.confirmedNewPassword && pwForm.newPassword !== pwForm.confirmedNewPassword
                            ? "border-red-400 focus:ring-red-400/10"
                            : pwForm.confirmedNewPassword && pwForm.newPassword === pwForm.confirmedNewPassword
                            ? "border-emerald-400 focus:ring-emerald-400/10"
                ...
                <div className="flex items-center gap-3 pt-1">
                  ...
                  <button type="submit" disabled={isChangingPw || (pwForm.confirmedNewPassword && pwForm.newPassword !== pwForm.confirmedNewPassword)}
                    className="flex-[1.5] flex items-center justify-center gap-2 py-3 bg-amber-500 hover:bg-amber-600 text-white text-sm font-bold rounded-2xl transition-all shadow-lg shadow-amber-500/20 disabled:opacity-50">
